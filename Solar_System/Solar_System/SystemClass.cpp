...
bool SystemClass::Frame()
{
	...
	bool keyDown = m_Input->IsRightArrowPressed();
	m_Position->MoveFrontZ(keyDown);

	keyDown = m_Input->IsLeftArrowPressed();
	m_Position->MoveBackZ(keyDown);

	keyDown = m_Input->IsUpArrowPressed();
	m_Position->TurnRightX(keyDown);

	keyDown = m_Input->IsDownArrowPressed();
	m_Position->TurnLeftX(keyDown);

	float m_rotationX, m_positionZ;

	m_Position->GetRotationX(m_rotationX);
	m_Position->GetPositionZ(m_positionZ);

	// 그래픽 객체의 Frame을 처리합니다
	return m_Graphics->Frame(m_positionZ, m_rotationX);
}
...
void SystemClass::InitializeWindows(int& screenWidth, int& screenHeight)
{
	...
	// 프로그램 이름을 지정합니다
	m_applicationName = L"Solar_System";
